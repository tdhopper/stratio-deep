...
public final class DeepJobConfig<T extends IDeepType> implements IDeepJobConfig<T> {
  ...
  /**
   * the name of the output keyspace. Can be null.
   */
  private String outputKeyspace;

  /**
   * the name of the output CF. Can be null.
   */
  ...
  public IDeepJobConfig<T> initialize() {
    ...
    try {
      ...
      if (StringUtils.isNotEmpty(outputColumnFamily)) {
        ConfigHelper.setOutputColumnFamily(c, outputColumnFamily);

        CqlConfigHelper.setOutputCql(c,
            updateQueryGenerator(entityClass, ConfigHelper.getOutputKeyspace(c), ConfigHelper.getOutputColumnFamily(c)));
      }
