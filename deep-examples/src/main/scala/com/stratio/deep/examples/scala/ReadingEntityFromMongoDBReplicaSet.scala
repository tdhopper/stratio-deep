...
final object ReadingEntityFromMongoDBReplicaSet {
  ...
  def doMain(args: Array[String]) {
    ...
    val inputConfigEntity: IMongoDeepJobConfig[MessageTestEntity] = MongoConfigFactory.createMongoDB(classOf[MessageTestEntity]).host(host1).host(host2).host(host3).database(database).collection(inputCollection).replicaSet(replicaSet).readPreference(readPreference).initialize
