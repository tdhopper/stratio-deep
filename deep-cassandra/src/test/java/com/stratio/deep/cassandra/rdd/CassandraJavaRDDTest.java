...
public final class CassandraJavaRDDTest extends AbstractDeepSparkContextTest {
    ...
    protected void initServerAndRDD() throws IOException, URISyntaxException, ConfigurationException,
        ...
        rddConfig = new DeepJobConfig<>(new ExtractorConfig(TestEntity.class));
    ...
    public void testSaveToCassandra() {
        ...
        DeepJobConfig<Cells> writeConfig = new DeepJobConfig<>(new ExtractorConfig<>(Cells.class));
        Map<String, Serializable> values = new HashMap<>();
        values.put(ExtractorConstants.HOST, Constants.DEFAULT_CASSANDRA_HOST);
        values.put(ExtractorConstants.RPCPORT, CassandraServer.CASSANDRA_THRIFT_PORT);
        ...
        writeConfig.getExtractorConfiguration().setValues(values);
        writeConfig.getExtractorConfiguration().setExtractorImplClass(CassandraCellExtractor.class);
...
class Tuple2CellsFunction implements Function<Tuple2<String, Integer>, Cells> {
    ...
    public Cells call(Tuple2<String, Integer> t) throws Exception {
        return new Cells("defaultTable", CassandraCell.create("domain", t._1(), true, false), CassandraCell.create(
                "count", t._2()));
    }
}
