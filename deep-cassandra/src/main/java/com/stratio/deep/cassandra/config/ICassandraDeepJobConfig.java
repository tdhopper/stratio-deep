...
public interface ICassandraDeepJobConfig<T> extends IDeepJobConfig<T, ICassandraDeepJobConfig<T>> {
     ...
     * Once a new filter has been added, all subsequent queries generated to the underlying datastore will include the
     * filter on the specified column called <i>filterColumnName</i>. Before propagating the filter we check if an index
     * exists in Cassandra.
     * 
     * @param filterColumnName
     *            the name of the columns (as known by the datastore) to filter on.
     * @param filterValue
     *            the value of the filter to use. May be any expression, depends on the actual index implementation.
     * @return this configuration object.
     * @throws com.stratio.deep.commons.exception.DeepIndexNotFoundException
     *             if the specified field has not been indexed in Cassandra.
     * @throws com.stratio.deep.commons.exception.DeepNoSuchFieldException
    ...
    /**
     * RPC port where the remote Cassandra cluster is listening to. Defaults to 9160.
     * 
    ...
    /**
     * CQL port where the remote Cassandra cluster is listening to. Defaults to 9042.
     * 
     * @return the cql port.
     */
    ...
    /**
     * Sets the token range bisect factor. The provided number must be a power of two. Defaults to 1.
     * 
     * @param bisectFactor
    ...
    /**
     * Sets the cassandra CF from which data will be read from. Column family name is case sensitive.
     * 
     * @param columnFamily
     ...
     *            a boolean that tells this configuration obj to create missing tables on write.
     * @return this configuration object.
     */
