...
public class MongoCellExtractorTest extends ExtractorTest {
    ...
    public void testDataSet() {
        ...
        DeepJobConfig<Cells> inputConfigEntity = new DeepJobConfig<>(new ExtractorConfig(Cells.class));
        ...
        assertEquals(((DBObject) result.get("metadata")).get("author"), ((Cells) book.getCellByName("metadata")
                .getCellValue()).getCellByName("author").getCellValue());
        ...
        for (int i = 0; i < listCantos.size(); i++) {
            ...
            assertEquals(bsonObject.get("canto"), ((List<Cells>) book.getCellByName("cantos").getCellValue()).get(i)
                    .getCellByName("canto").getCellValue());
            assertEquals(bsonObject.get("text"), ((List<Cells>) book.getCellByName("cantos").getCellValue()).get(i)
                    .getCellByName("text").getCellValue());
        }
        ...
        JavaRDD<Cells> outputRDD = wordCountReduced.map(new Function<Tuple2<String, Integer>, Cells>() {
            ...
            public Cells call(Tuple2<String, Integer> stringIntegerTuple2) throws Exception {
                return new Cells(MongoCell.create("word", stringIntegerTuple2._1()), MongoCell.create("count",
                        stringIntegerTuple2._2()));
            }
        });

        DeepJobConfig<Cells> outputConfigEntity = new DeepJobConfig<>(new ExtractorConfig<>(Cells.class));
